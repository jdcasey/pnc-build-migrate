...
def promote_build(build, group, config, pod, progress_file):
    status = do_promote(build, config, progress_file)
...
def promote_builds(config, input_file, progress_file):
    ...
    group = get_group(config, progress_file)
    if group is None:
    ...
    if target_key not in group['constituents']:
        ...
        success = update_group(group, config, progress_file)
        if success is False:
